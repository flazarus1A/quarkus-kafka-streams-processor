...
class PingProcessorQuarkusTest {
    ...
    String globalDataTopic = "global-data-topic";

    KafkaProducer<String, Ping> producerPing;

    KafkaProducer<String, String> producerGlobalDataTopic;
    ...
    public void setup() {
        ...
        producerGlobalDataTopic = new KafkaProducer<>(producerProps, new StringSerializer(),
    ...
    void testGlobalStoreValueRetrieval() throws InterruptedException {
        ...
        producerGlobalDataTopic.send(new ProducerRecord<>(globalDataTopic, "ID1", "value1"));
        Thread.sleep(500L);
        producerPing.send(new ProducerRecord<>(senderTopic, "ID1", Ping.newBuilder().setMessage("whatever").build()));
        ...
        assertThat(receivedRecord.value().getMessage(), containsString("Stored value for ID1 is VALUE1"));

        producerPing.send(new ProducerRecord<>(senderTopic, "ID1", Ping.newBuilder().setMessage("whatever").build()));
        receivedRecord = KafkaTestUtils.getSingleRecord(consumer, consumerTopic);
        assertThat(receivedRecord.value().getMessage(), containsString("Stored value for ID1 is VALUE1"));

        producerGlobalDataTopic.send(new ProducerRecord<>(globalDataTopic, "ID1", "value2"));
        Thread.sleep(500L);
        producerPing.send(new ProducerRecord<>(senderTopic, "ID1", Ping.newBuilder().setMessage("whatever").build()));
        receivedRecord = KafkaTestUtils.getSingleRecord(consumer, consumerTopic);
        // Check that the value has been stored in the global store
        assertThat(receivedRecord.value().getMessage(), containsString("Stored value for ID1 is VALUE2"));
    }
}
