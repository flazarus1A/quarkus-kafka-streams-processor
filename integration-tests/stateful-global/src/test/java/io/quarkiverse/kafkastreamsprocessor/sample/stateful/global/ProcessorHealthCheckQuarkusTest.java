...
public class ProcessorHealthCheckQuarkusTest {
    ...
    public void testHealthChecksDuringLifecycle() throws Exception {
        String globalStoreTopic = kStreamsProcessorConfig.globalStores().topics().get("ping-data");
        ...
        try (AdminClient admin = AdminClient
            ...
            admin.deleteTopics(Collections.singletonList(globalStoreTopic)).all().get(
                    10L,
                    TimeUnit.SECONDS);
        }
        ...
        checkReadiness().responseCode(503)
                .isUp(false)
                .topicsCheck(false,
                        String.join(",", inputTopic, outputTopic),
                        globalStoreTopic)
                .stateCheck(true, "RUNNING");
